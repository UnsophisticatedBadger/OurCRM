...
# ── keyboard shortcuts Ctrl+2 through Ctrl+6 ──────────────────────────────────


def test_ctrl_1_navigates_to_contacts(qtbot: QtBot) -> None:
    w = _make(qtbot)
    w.navigate_to(Section.LEADS)
...
def test_ctrl_2_navigates_to_leads(qtbot: QtBot) -> None:
    w = _make(qtbot)
    qtbot.keyClick(w, Qt.Key.Key_2, modifier=Qt.KeyboardModifier.ControlModifier)  # type: ignore[no-untyped-call]
    assert w.current_section() == Section.LEADS


def test_ctrl_3_navigates_to_properties(qtbot: QtBot) -> None:
    w = _make(qtbot)
...
def test_ctrl_4_navigates_to_transactions(qtbot: QtBot) -> None:
    w = _make(qtbot)
...
def test_ctrl_5_navigates_to_calendar(qtbot: QtBot) -> None:
    w = _make(qtbot)
...
def test_ctrl_6_navigates_to_settings(qtbot: QtBot) -> None:
    w = _make(qtbot)
...
def test_ctrl_7_does_not_change_section(qtbot: QtBot) -> None:
    w = _make(qtbot)
