"""Unit tests for NavigationPanel and section switching."""
...
def test_has_seven_items(qtbot: QtBot) -> None:
    assert _make(qtbot).count() == 7
...
def test_default_section_is_dashboard(qtbot: QtBot) -> None:
    assert _make(qtbot).current_section() == Section.DASHBOARD
