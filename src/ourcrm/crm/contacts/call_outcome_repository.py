...
class CallOutcome:
    ...
    callback_start_date: date | None = None
    callback_end_date: date | None = None
    id: int | None = None
...
def _to_domain(row: CallOutcomeRow) -> CallOutcome:
    return CallOutcome(
        ...
        callback_start_date=row.callback_start_date,
        callback_end_date=row.callback_end_date,
        id=row.id,
    )
    ...
    def log(
        ...
        with self._session_factory() as session:
            row = CallOutcomeRow(
                ...
                callback_start_date=callback_start,
                callback_end_date=callback_end,
            )
            session.add(row)
            session.commit()
            session.refresh(row)
